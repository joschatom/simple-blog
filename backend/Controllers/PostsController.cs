...
public class PostsController(IPostRepository repository, IMapper mapper) : ControllerBase
{
    ...
    [HttpPut("{id}")]
    public async Task<ActionResult> Put(Guid id, [FromBody] UpdatePostDTO update)
    {
        if (!ModelState.IsValid)
            return BadRequest(ModelState);
        ...
        mapper.Map(update, existingPost);

        await repository.UpdateAsync(existingPost.Id, existingPost);
        await repository.SaveChangesAsync();
